...
const Palettes = (props) => {
    ...
    return (
        <div className="color-white mt-[10px] rounded-md px-2.5 h-[200px] w-[300px] bg-white cursor-pointer">
            ...
            <div className="grid-cols-5 grid-rows-4 w-full h-[155px] mt-2.5 pt-2.5 grid">
                {colors.map((color, index) => (
                    <div key={index} className="w-full h-full" style={{ backgroundColor: color.color }}/>
                ))}
            </div>

            <a aria-label='palette' className='text-[18px] mt-[2px] text-[#000] font-semibold h-[40px] leading-[40px] float-left w-full text-left [text-indent:2px] tracking-[.55px]'>
